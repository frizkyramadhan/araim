              ...
              <div class="card-tools">
                <ul class="nav nav-pills ml-auto">
                  <li class="nav-item mr-2">
                    <a class="btn btn-primary" href="{{ url('employees/' . $employee->id . '/edit') }}"><i class="fas fa-pen-square"></i>
                      Edit</a>
            ...
            <div class="card-body">
              <div class="tab-content p-0">
                ...
                <div class="form-group row">
                  ...
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="position_id" value="{{ $employee->position->position_name }}" disabled>
                  </div>
                </div>
                <div class="form-group row">
                  <label class="col-sm-2 col-form-label">Project</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="project_id" value="{{ $employee->project->project_code }} - {{ $employee->project->project_name }}" disabled>
                  </div>
                </div>
                <div class="form-group row">
                ...
                <div class="form-group">
                  ...
                  <div class="float-right">
                    <a class="btn btn-success" href="{{ url('inventories/print_qrcode_employee/' . $employee->id) }}" target="_blank"><i class="fas fa-qrcode"></i>
                      Print</a>
                    <a class="btn btn-warning" href="{{ url('inventories/create/' . $employee->id) }}"><i class="fas fa-plus"></i>
                      Add</a>
                  </div>
                </div>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td class="text-center">
                          ...
                          <a href="{{ url('inventories/qrcode/' . $inventory->id) }}" class="btn btn-xs btn-secondary"><i class="fas fa-qrcode"></i> Generate</a>
                        </td>
                        <td>{{ $inventory->input_date }}</td>
                        <td>{{ $inventory->asset_name }}</td>
                        <td>{{ $inventory->brand_name ? $inventory->brand_name : $inventory->brand  }} - {{ $inventory->model_asset }}</td>
                        ...
                        <td class="text-center">
                          ...
                          <a href="{{ url('inventories/delete_qrcode/' . $inventory->id) }}" class="btn btn-xs btn-danger" onclick="return confirm('Are you sure to delete this qrcode?')">
                        ...
                        <td class="text-center">
                          <a title="Detail" class="btn btn-icon btn-success" href="{{ url('inventories/' .$inventory->id.'/'.$employee->id) }}"><i class="fas fa-info-circle"></i></a>
                          <a class="btn btn-icon btn-primary" href="{{ url('inventories/edit/' . $inventory->id . '/'. $employee->id) }}"><i class="fas fa-pen-square"></i></a>
                          <form action="{{ url('inventories/' . $inventory->id .'/'.$employee->id) }}" method="post" onsubmit="return confirm('Are you sure want to delete this data?')" class="d-inline">
                            @method('delete')
                            @csrf
