                                ...
                                <div class="card-tools">
                                    <ul class="nav nav-pills ml-auto">
                                        <li class="nav-item mr-2">
                                            <a class="btn btn-warning" href="{{ url('bapbs') }}"><i
                                ...
                                <div class="card-body">
                                    <div class="tab-content p-0">
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="hidden"
                                                    class="form-control @error('bapb_no') is-invalid @enderror"
                                                    name="bapb_no" placeholder="BAPB No." value="{{ $bapb_no }}"
                                                    required readonly>
                                                <input type="text"
                                                    class="form-control @error('bapb_reg') is-invalid @enderror"
                                                    name="bapb_reg" placeholder="BAPB No."
                                                    value="{{ $bapb_no }}/BAPB/ITY/{{ $month }}/{{ $year }}"
                                                    required readonly>
                                                @error('bapb_reg')
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <input type="date"
                                                    class="form-control @error('bapb_date') is-invalid @enderror"
                                                    name="bapb_date" value="{{ old('bapb_date') }}">
                                                @error('bapb_date')
                                                    <div class="error invalid-feedback">
                                                        {{ $message }}
                                                    </div>
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <select name="bapb_submit"
                                                    class="form-control @error('bapb_submit') is-invalid @enderror select2bs4"
                                                    style="width: 100%;">
                                                    <option value="">-- Select Employee --</option>
                                                    @foreach ($submits as $submit)
                                                        <option value="{{ $submit->id }}"
                                                            {{ old('bapb_submit') == $submit->id ? 'selected' : '' }}>
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                <select id="bapb_receive" name="bapb_receive"
                                                    class="form-control @error('bapb_receive') is-invalid @enderror select2bs4"
                                                    style="width: 100%;">
                                                    <option value="">-- Select Employee --</option>
                                                    @foreach ($receives as $receive)
                                                        <option value="{{ $receive->id }}"
                                                            {{ old('bapb_receive') == $receive->id || (isset($selectedEmployeeId) && $selectedEmployeeId == $receive->id) ? 'selected' : '' }}>
...
@section('scripts')
    ...
    <script>
        ...
        function loadInventories(employeeId, selectedInventoryId) {
            ...
            $.ajax({
                ...
                success: function(inventories) {
                    ...
                    $.each(inventories, function(index, inventory) {
                        var isChecked = selectedInventoryId && selectedInventoryId == inventory.id ?
                            'checked' : '';
                        var row = '<tr>';
                        row += '<td><input type="checkbox" name="inventory_id[]" id="' +
                            inventory.id + '" value="' + inventory.id + '" ' + isChecked + '></td>';
                        ...
                        row += '<td>' + (inventory.project_code || '-') + '</td>';
        ...
        $('#bapb_receive').on('change', function() {
            ...
            loadInventories(bapb_receive, null);
        });

        // Auto-load inventories if selectedEmployeeId is present
        @if (isset($selectedEmployeeId) && $selectedEmployeeId)
            $(document).ready(function() {
                var selectedEmployeeId = {{ $selectedEmployeeId }};
                var selectedInventoryId = {{ $selectedInventoryId ?? 'null' }};
                if (selectedEmployeeId) {
                    loadInventories(selectedEmployeeId, selectedInventoryId);
                }
            });
        @endif
    </script>
